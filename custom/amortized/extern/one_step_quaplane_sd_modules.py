...
class QuaplaneLoRAConv2dLayer(nn.Module):
    ...
    def _init_weights(self):
        for layer in [
            ...
            "down_side",
            "down_front",
            "down_back",  # in case of quadra_vX
            "up_overhead",
            "up_side",
            "up_front",
            "up_back" "down",  # in case of quadra_vX
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        orig_dtype = hidden_states.dtype
        dtype = (
            self.down_overhead.weight.dtype
        ...
        if "quadra" in self.locon_type:
            ...
            up_hidden_states = torch.concat(
                [torch.zeros_like(up_hidden_states_front)] * 4, dim=0
            )
...
class QuaplaneSelfAttentionLoRAAttnProcessor(nn.Module):
    ...
    def __init__(
        ...
        if lora_type in ["quadra_v1", "sparse_v2"]:
            ...
            self.to_q_overhead_lora = LoRALinearLayerwBias(
                hidden_size, hidden_size, rank, network_alpha, with_bias=with_bias
            )
            self.to_k_overhead_lora = LoRALinearLayerwBias(
                hidden_size, hidden_size, rank, network_alpha, with_bias=with_bias
            )
            self.to_v_overhead_lora = LoRALinearLayerwBias(
                hidden_size, hidden_size, rank, network_alpha, with_bias=with_bias
            )
            self.to_out_overhead_lora = LoRALinearLayerwBias(
                hidden_size, hidden_size, rank, network_alpha, with_bias=with_bias
            )
    ...
    def __call__(
        ...
        if self.lora_type in ["quadra_v1", "sparse_v1", "sparse_v2"]:
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for the query in QuaplaneSelfAttentionLoRAAttnProcessor."
            )
        ...
        if self.lora_type in ["quadra_v1", "sparse_v2"]:
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for the key and value in QuaplaneSelfAttentionLoRAAttnProcessor."
            )
        ...
        if self.lora_type in ["quadra_v1", "vanilla"]:
            ...
            hidden_states = hidden_states.view(
                batch_size // 4 * 4, sequence_length, self.hidden_size
            )
            ...
            _value_append[0::4] = value[
                1::4
            ]  # overhead view is appended with side view
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for attention scores calculation in QuaplaneSelfAttentionLoRAAttnProcessor."
            )
        ...
        if self.lora_type in ["quadra_v1", "sparse_v2"]:
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for the to_out layer in QuaplaneSelfAttentionLoRAAttnProcessor."
            )
...
class QuaplaneCrossAttentionLoRAAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        if self.lora_type in ["quadra_v1"]:
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for the query in QuaplaneCrossAttentionLoRAAttnProcessor."
            )
        ...
        if self.lora_type in ["quadra_v1"]:
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for the key and value in QuaplaneCrossAttentionLoRAAttnProcessor."
            )
        ...
        if self.lora_type in ["quadra_v1"]:
            ...
            raise NotImplementedError(
                "The LoRA type is not supported for the to_out layer in QuaplaneCrossAttentionLoRAAttnProcessor."
            )
