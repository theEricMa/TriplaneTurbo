...
class GenerativeSpaceDmtetRasterizeRenderer(NVDiffRasterizer):
    ...
    class Config(NVDiffRasterizer.Config):
        ...
        sdf_grad_shrink: float = 1.0

        def_grad_shrink: float = 1.0
    ...
    def configure(
        ...
        assert self.cfg.isosurface_method in [
            "mt",
            "mc-cpu",
            "diffmc",
        ], "Invalid isosurface method"
    ...
    def forward(
        ...
        **kwargs,
        ...
        for batch_idx, mesh in enumerate(mesh_list):
            _mvp_mtx: Float[Tensor, "B 4 4"] = mvp_mtx[
            ...
            if self.training:  # requires only 4 views, memory efficient:
                ...
                for i in range(0, v_pos_clip.shape[0], n_views_per_rasterize):
                    rast, _ = self.ctx.rasterize(
                        v_pos_clip[i : i + n_views_per_rasterize],
                        mesh.t_pos_idx,
                        (height, width),
                    )
                    rast_list.append(rast)
                    gb_feat, _ = self.ctx.interpolate(
                        v_pos_clip[i : i + n_views_per_rasterize], rast, mesh.t_pos_idx
                    )
            ...
            disparity_norm = torch.lerp(
                torch.zeros_like(depth), disparity_norm, mask.float()
            )
            disparity_norm = self.ctx.antialias(
                disparity_norm, rast, v_pos_clip, mesh.t_pos_idx
            )
            ...
            if self.cfg.normal_direction == "camera":
                ...
                w2c: Float[Tensor, "B 4 4"] = torch.inverse(
                    c2w[
                        (batch_idx)
                        * num_views_per_batch : (batch_idx + 1)
                        * num_views_per_batch
                    ]
                )
                ...
                gb_normal_cam = (
                    gb_normal[..., None, :]
                    @ rotate.permute(0, 2, 1)[..., None, None, :, :]
                )
                flip_x = torch.eye(3).to(
                    w2c
                ...
                camera_gb_normal_bg = torch.lerp(bg_normal, gb_normal_cam, mask.float())
                ...
                out.update(
                    {
                        ...
                        if not is_emtpy
                        else camera_gb_normal_bg.detach(),
                        "comp_normal_cam_vis_white": camera_gb_normal_bg_white
                        if not is_emtpy
                        else camera_gb_normal_bg_white.detach(),
                    }
                )
            ...
            if render_rgb:
                ...
                if torch.is_tensor(space_cache):  # space cache
                    ...
                    for key in space_cache.keys():
                        space_cache_slice[key] = [
                            weight[batch_idx : batch_idx + 1]
                            for weight in space_cache[key]
                        ]
                ...
                if self.material.requires_normal:
                    extra_geo_info["shading_normal"] = (
                        gb_normal[selector]
                        if not is_emtpy
                        else gb_normal[selector].detach()
                    )
                ...
                if self.material.requires_tangent:
                    ...
                    extra_geo_info["tangent"] = (
                        gb_tangent[selector]
                        if not is_emtpy
                        else gb_tangent[selector].detach()
                    )
                ...
                if "sdf_grad" in geo_out:
                    if "sdf_grad" in out:
                        out["sdf_grad"].extend(geo_out["sdf_grad"])
                    else:
                        out["sdf_grad"] = geo_out["sdf_grad"]
                if "sdf" in geo_out:
                    if "sdf" in out:
                        out["sdf"].extend(geo_out["sdf"])
                    else:
                        out["sdf"] = geo_out["sdf"]
                ...
                gb_rgb_fg = torch.zeros(num_views_per_batch, height, width, 3).to(
                    rgb_fg
                )
    ...
    def isosurface(self, space_cache: Float[Tensor, "B ..."]) -> List[Mesh]:
        ...
        if self.sdf_grad_shrink != 0:
            sdf_batch = (
                self.sdf_grad_shrink * sdf_batch
                + (1 - self.sdf_grad_shrink) * sdf_batch.detach()
            )
        else:  # save memory
        ...
        if self.def_grad_shrink != 0:
            deformation_batch = (
                self.sdf_grad_shrink * deformation_batch
                + (1 - self.sdf_grad_shrink) * deformation_batch.detach()
                if deformation_batch is not None
                else None
            )
        else:  # save memory
            deformation_batch = (
                deformation_batch.detach() if deformation_batch is not None else None
            )
        ...
        for index in range(sdf_batch.shape[0]):
            ...
            if torch.all(sdf > 0) or torch.all(sdf < 0):
                threestudio.info(
                    "All sdf values are positive or negative, no isosurface"
                )
                self.empty_flag = (
                    self.cfg.allow_empty_flag
                )  # special operation, set to detach the gradient from wrong isosurface
            ...
            if index > 0 and self.cfg.isosurface_method == "diffmc":
                ...
                if not hasattr(self, f"isosurface_helper_{index}"):
                    ...
                    setattr(
                        self,
                        f"isosurface_helper_{index}",
                        DiffMarchingCubeHelper(
                            self.cfg.isosurface_resolution,
                        ),
                    )
