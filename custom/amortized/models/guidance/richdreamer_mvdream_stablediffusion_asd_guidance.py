...
@threestudio.register(
    "richdreamer-mvdream-stablediffusion-asynchronous-score-distillation-guidance"
)
class RDMVASDsynchronousScoreDistillationGuidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        rd_recon_std_rescale: float = 0.0
        ...
        **kwargs,
        ...
        text_embeddings = torch.cat(
            [
                text_embeddings_vd
                if not is_dual
                else text_embeddings_vd.repeat(2, 1, 1),
                text_embeddings_uncond
                if not is_dual
                else text_embeddings_uncond.repeat(2, 1, 1),
                text_embeddings_vd
                if not is_dual
                else text_embeddings_vd.repeat(2, 1, 1),
            ],
        ...
        if self.cfg.mv_weighting_strategy in [
            "sds",
            "uniform",
            "fantasia3d",
            "sds_sqrt",
            ...
            with torch.no_grad():
                ...
                if self.cfg.mv_weighting_strategy == "dmd":
                    ...
                    w = torch.abs(mv_latents - _latent_second).mean(
                        dim=(1, 2, 3), keepdim=True
                    )
            ...
            grad = (latent_second - latent_first) / (
                w + self.cfg.eps
            )  # avoid zero division
        ...
        if not is_dual:
            loss_asd = (
                0.5 * F.mse_loss(mv_latents, target, reduction="sum") / self.cfg.n_view
            )
    ...
    def _rd_noise_pred(
        ...
        if use_t_plus:
            ...
            noise_pred_text, noise_pred_uncond = noise_pred.chunk(2)
            noise_pred_text_second = (
                noise_pred_text if not self.cfg.rd_use_sds else rd_noise
            )
    ...
    def _rd_grad_asd(
        ...
        **kwargs,
        ...
        if self.cfg.rd_weighting_strategy in [
            "sds",
            "uniform",
            "fantasia3d",
            "sds_sqrt",
            ...
            if self.cfg.rd_recon_std_rescale > 0:
                ...
                latents_noisy = self.rd_model.q_sample(rd_latents, t, noise=rd_noise)
                latents_recon = self.rd_model.predict_start_from_noise(
                    latents_noisy, t, noise_pred_first
                )
                # rescale the reconstruction
                latents_recon_nocfg = self.rd_model.predict_start_from_noise(
                    latents_noisy, t, noise_pred_text
                )
                factor = (
                    latents_recon_nocfg.view(
                        -1, self.cfg.n_view, *latents_recon_nocfg.shape[1:]
                    ).std(dim=[1, 2, 3, 4], keepdim=True)
                    + 1e-8
                ) / (
                    latents_recon.view(
                ...
                latents_recon = (
                    self.cfg.rd_recon_std_rescale
                    * (
                        latents_recon.clone()
                        * factor.squeeze(1).repeat_interleave(self.cfg.n_view, dim=0)
                    )
                ...
                if use_t_plus:
                    ...
                    latents = self.rd_model.predict_start_from_noise(
                        latents_noisy_second, t_plus, noise_pred_second
                    )
                else:
                    latents = rd_latents
            ...
            with torch.no_grad():
                ...
                if self.cfg.rd_weighting_strategy == "dmd":
                    w = torch.abs(rd_latents - latent_first).mean(
                        dim=(1, 2, 3), keepdim=True
                    )
                    ...
                    _noise_pred_second = (
                        noise_pred_uncond
                        + self.cfg.rd_guidance_scale
                        * (noise_pred_text_second - noise_pred_uncond)
                    )
                    _alpha = (self.alphas[t_plus] ** 0.5).view(-1, 1, 1, 1)
                    _sigma = ((1 - self.alphas[t_plus]) ** 0.5).view(-1, 1, 1, 1)
                    _latent_second = (rd_latents - _sigma * _noise_pred_second) / _alpha
                    w = torch.abs(rd_latents - _latent_second).mean(
                        dim=(1, 2, 3), keepdim=True
                    )
        ...
        if not is_dual:
            loss_asd = (
                0.5 * F.mse_loss(rd_latents, target, reduction="sum") / self.cfg.n_view
            )
            return loss_asd, grad.norm()
        else:
    ...
    def _sd_grad_asd(
        ...
        **kwargs,
        ...
        # prepare text embeddings
        _, text_embeddings = prompt_utils.get_text_embeddings(
            use_default_neg=True
        )  # sdreamer uses the second text embeddings and the default negative text: ""
        text_batch_size = text_embeddings.shape[0] // 2
        ...
        if self.cfg.sd_weighting_strategy in [
            "sds",
            "uniform",
            "fantasia3d",
            "sds_sqrt",
        ...
        if not is_dual:
            loss_asd = 0.5 * F.mse_loss(
                sd_latents, target, reduction="sum"
            )  # / view_batch_size
            return loss_asd, grad.norm()
        else:
            # split the grad into two parts
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.cfg.sd_hq_only:
            assert (
                self.cfg.sd_oppo_side
            ), "sd_oppo_side must be True if sd_hq_only is True"
            # select only one rendering from n_view renderings
            idx = torch.randint(
                0,
                self.cfg.n_view,
                (rgb.shape[0] // self.cfg.n_view,),
                device=self.device,
                dtype=torch.long,
            )
            _idx = torch.arange(
                0, rgb.shape[0], self.cfg.n_view, device=self.device, dtype=torch.long
            )
            if is_dual:
                idx_2nd = (
                    idx + self.cfg.n_view // 2
                ) % self.cfg.n_view  # the 2nd rendering is the opposite view, we assume n_view is even
            ...
            if self.cfg.sd_oppo_side:
                ...
                if is_dual:
                    idx_2nd = torch.randint(
                        0,
                        self.cfg.n_view,
                        (rgb_2nd.shape[0] // self.cfg.n_view,),
                        device=self.device,
                        dtype=torch.long,
                    )
                    idx_2nd += torch.arange(
        ...
        if self.sd_weight > 0:
            loss_sd, grad_sd = self.sd_grad_asd(
                ...
                rgb_as_latents=rgb_as_latents,
                fovy=fovy,
                rgb_2nd=rgb_2nd[idx_2nd]
                if is_dual and not self.cfg.sd_hq_only
                else None,
        ...
        if not is_dual:
            return {
                "loss_asd": self.rd_weight * loss_rd
                + self.mv_weight * loss_mv
                + self.sd_weight * loss_sd,
                "grad_norm_asd": self.rd_weight * grad_rd
                + self.mv_weight * grad_mv
                + self.sd_weight * grad_sd,
            }
        else:
            ...
            if (
                not self.cfg.sd_hq_only
            ):  # otherwise, sd's grad for the 2nd rendering is not computed
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        self.sd_min_step = (
            int(
                ...
                * C(self.cfg.sd_min_step_percent, epoch, global_step)
            )
            if self.cfg.sd_min_step_percent is not None
            else min_step
        )
        self.sd_max_step = (
            int(
                self.num_train_timesteps
                * C(self.cfg.sd_max_step_percent, epoch, global_step)
            )
            if self.cfg.sd_max_step_percent is not None
            else max_step
        )
        # rich dreamer
        ...
        self.rd_min_step = (
            int(
                self.num_train_timesteps
                * C(self.cfg.rd_min_step_percent, epoch, global_step)
            )
            if self.cfg.rd_min_step_percent is not None
            else min_step
        )
        self.rd_max_step = (
            int(
                self.num_train_timesteps
                * C(self.cfg.rd_max_step_percent, epoch, global_step)
            )
            if self.cfg.rd_max_step_percent is not None
            else max_step
        )
        # mvdream
        ...
        self.mv_min_step = (
            int(
                self.num_train_timesteps
                * C(self.cfg.mv_min_step_percent, epoch, global_step)
            )
            if self.cfg.mv_min_step_percent is not None
            else min_step
        )
        self.mv_max_step = (
            int(
                self.num_train_timesteps
                * C(self.cfg.mv_max_step_percent, epoch, global_step)
            )
            if self.cfg.mv_max_step_percent is not None
            else max_step
        )
