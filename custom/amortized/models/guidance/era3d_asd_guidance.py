...
import threestudio
from extern.era3D.pipeline.pipeline_mvdiffusion_unclip import (
...
class Era3DAsynchronousScoreDistillationGuidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        era3d_weight: float = 1.0
        era3d_color_weight: float = 1.0
        era3d_normal_weight: float = 1.0
    ...
    def configure(self) -> None:
        ...
        pipe = StableUnCLIPImg2ImgPipeline.from_pretrained(
            self.cfg.era3d_model_name_or_path, torch_dtype=self.weight_dtype
        )
        try:
            self.normal_text_embeds = torch.load(
                f"{self.cfg.era3d_embeddings_dir}/normal_embeds.pt"
            ).to(self.device)
            self.color_text_embeds = torch.load(
                f"{self.cfg.era3d_embeddings_dir}/clr_embeds.pt"
            ).to(self.device)
        except:
    ...
    def get_t_plus(self, t: Float[Tensor, "B"], module: str):  # "rd" or "mv"
        ...
        # add the offset
        if plus_random:
            t_delta = (t_delta * torch.rand(*t.shape, device=self.device)).to(
                torch.long
            )
        else:
    ...
    def era3d_get_latents(
        ...
        if not rgb_as_latents:
            ...
            posterior = self.era3d_vae.encode(latents.to(self.weight_dtype)).latent_dist
    ...
    def _era3d_noise_pred(
        ...
        if use_t_plus:
            ...
            ) = torch.chunk(noise_pred, 6, dim=0)
        else:
            (
                normal_noise_pred_cond,
                normal_noise_pred_uncond,
    ...
    def _era3d_grad_asd(
        ...
        **kwargs,
        ...
        era3d_latents = self.era3d_get_latents(
            ...
            rgb_2nd_BCWH=rgb_2nd.permute(0, 3, 1, 2) if is_dual else None,
            normal_2nd_BCWH=normal_2nd.permute(0, 3, 1, 2) if is_dual else None,
        )
        ...
        """
        # N_view, c <-- 1, c
        image_embeddings_cond = image_embeddings.repeat_interleave(
        ...
        with torch.no_grad():
            ...
            if use_t_plus:
                ...
                t_plus = _t_plus.repeat_interleave(self.cfg.n_view).repeat(
                    2
                )  # normal and color
            ...
            (
                ...
                color_noise_pred_cond,
                color_noise_pred_uncond,
                color_noise_pred_2nd,
            ) = self._era3d_noise_pred(
                era3d_latents,
                era3d_noise,
                image_embeddings_cond,
                image_embeddings_uncond,
            ...
            normal_noise_pred_1st = (
                normal_noise_pred_uncond
                + self.cfg.era3d_guidance_scale_normal
                * (normal_noise_pred_cond - normal_noise_pred_uncond)
            )
            color_noise_pred_1st = (
                color_noise_pred_uncond
                + self.cfg.era3d_guidance_scale_color
        ...
        if self.cfg.era3d_weighting_strategy == "dmd":
            with torch.no_grad():
                ...
                w = torch.abs(normal_era3d_latents - normal_latents_first).mean(
                    dim=(1, 2, 3), keepdim=True
                )
                normal_grad = (normal_latents_second - normal_latents_first) / (
                    w + self.cfg.eps
                )
                ...
                w = torch.abs(color_era3d_latents - color_latents_first).mean(
                    dim=(1, 2, 3), keepdim=True
                )
                color_grad = (color_latents_second - color_latents_first) / (
                    w + self.cfg.eps
                )
        else:
            raise ValueError(
                f"Unknown weighting strategy: {self.cfg.era3d_weighting_strategy}"
            )
        ...
        **kwargs,
        ...
        if self.cfg.auto_grad:
            ...
            if rgb_2nd is not None:
                ...
                (
                    normal_loss_era3d,
                    normal_grad_norm_era3d,
                    color_loss_era3d,
                    color_grad_norm_era3d,
                ...
                grad_normal, grad_normal_2nd, grad_color, grad_color_2nd = torch_grad(
    ...
    def sample(
        ...
        **kwargs,
        ...
        do_classifier_free_guidance = (
            self.cfg.era3d_guidance_scale_color > 0
            or self.cfg.era3d_guidance_scale_normal > 0
        )
        condict = prompt_utils.get_image_encodings()
        image_latents: Float[Tensor, "1 C H W"] = condict["image_latents"]
        ...
        with torch.no_grad():
            for _, t in enumerate(
                tqdm(self.sample_scheduler.timesteps, desc="Sampling")
            ):
                # add noise
                latents = self.sample_scheduler.add_noise(
                    latents_orig, torch.randn_like(latents_orig), t
                )

                if do_classifier_free_guidance:
                    normal_latents, color_latents = torch.chunk(latents, 2, dim=0)
                    noisy_latents = torch.cat(
                        [normal_latents, normal_latents, color_latents, color_latents],
                        0,
                    )
                ...
                latent_model_input = torch.cat(
                    [noisy_latents, image_latents_input], dim=1
                )
                latent_model_input = self.sample_scheduler.scale_model_input(
                ...
                if do_classifier_free_guidance:
                    ...
                    normal_noise_pred = (
                        normal_noise_pred_uncond
                        + self.cfg.era3d_guidance_scale_normal
                        * (normal_noise_pred_text - normal_noise_pred_uncond)
                    )
                    color_noise_pred = (
                        color_noise_pred_uncond
                        + self.cfg.era3d_guidance_scale_color
                        * (color_noise_pred_text - color_noise_pred_uncond)
                    )
                    noise_pred = torch.cat([normal_noise_pred, color_noise_pred], 0)

                # update the latents
                latents_orig = self.sample_scheduler.step(
                    noise_pred, t, latents
                ).pred_original_sample

            # decode the latents to images
            images = self.era3d_vae.decode(
                latents_orig.to(self.weight_dtype)
                / self.era3d_vae.config.scaling_factor,
                return_dict=False,
            )[0]
            ...
            input_images = self.era3d_vae.decode(
                image_latents.to(self.weight_dtype)
                / self.era3d_vae.config.scaling_factor,
                return_dict=False,
            )[0]
        ...
        for normal, color, cond in zip(
            ...
            return_list += (
                [
                    {"type": "rgb", "img": color, "kwargs": {"data_format": "CHW"}},
                    {"type": "rgb", "img": normal, "kwargs": {"data_format": "CHW"}},
                    {"type": "rgb", "img": cond, "kwargs": {"data_format": "CHW"}},
                ],
            )
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        self.era3d_min_step = int(
            self.num_train_timesteps
            * C(self.cfg.era3d_min_step_percent, epoch, global_step)
        )
        self.era3d_max_step = int(
            self.num_train_timesteps
            * C(self.cfg.era3d_max_step_percent, epoch, global_step)
        )
